...
public class TabularNonNullValuesRenderer extends OutputRenderer {
    ...
    protected class TabularNonNullValuesLayout extends TabularLayout {
        ...
        private boolean validMultiLanguage(LocalizedString multiLanguageString) {
            for (Locale locale : multiLanguageString.getLocales()) {
                if (multiLanguageString.getContent(locale).trim().length() > 0) {
                    return true;
                }
            }
            return false;
