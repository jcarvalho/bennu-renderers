...
public class ViewState implements IViewState {
    ...
    public static String encodeToBase64(List<IViewState> viewStates) throws IOException {
        ...
        kryo.register(ViewDestination.class, new Serializer<ViewDestination>() {
            @Override
            public void write(Kryo kryo, Output output, ViewDestination object) {
                output.writeString(object.getPath());
                output.writeString(object.getModule());
                output.writeBoolean(object.getRedirect());
            }

            @Override
            public ViewDestination read(Kryo kryo, Input input, Class<ViewDestination> type) {
                return new ViewDestination(input.readString(), input.readString(), input.readBoolean());
            }
        });
        ...
        try {
            Output output = new UnsafeOutput(4096, Integer.MAX_VALUE);
            kryo.writeClassAndObject(output, viewStates);
            byte[] bytes = output.toBytes();
            output.close();
            System.out.println("Serializing: " + Base64.getEncoder().encodeToString(bytes));
            byte[] compressedBytes = compress(bytes);
            System.out.println("Compressed: " + Base64.getEncoder().encodeToString(compressedBytes));
        } catch (Exception e) {
            e.printStackTrace();
        }
